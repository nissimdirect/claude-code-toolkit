#!/usr/bin/env bash
# qwen-safe.sh — Hardened wrapper for Qwen Code CLI
# Blocks dangerous patterns before passing to real binary.
# Used by OpenClaw instead of raw `qwen` to prevent prompt injection → exec attacks.

set -euo pipefail

QWEN_BIN="/opt/homebrew/bin/qwen"
LOG_FILE="$HOME/.openclaw/logs/qwen-safe-audit.log"
...
check_prompt() {
    ...
    for pattern in "${BLOCKED_PATTERNS[@]}"; do
        if echo "$prompt" | grep -qEi "$pattern"; then
            ...
            echo "ERROR: Blocked by qwen-safe.sh — prompt contains restricted pattern." >&2
            echo "If this is legitimate, run qwen directly: $QWEN_BIN" >&2
            exit 1
        fi
    done
}
