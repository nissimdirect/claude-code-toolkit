...
DEPENDENCY_GRAPH = {
    "skill_added": {
        "description": "New skill file added to ~/.claude/skills/",
        "updates": [
    ...
    "skill_removed": {
        "description": "Skill file deleted from ~/.claude/skills/",
        "updates": [
    ...
    "article_count_changed": {
        ...
        "updates": [
            ...
            ("self-improve.skill", "Update Training Data Audit section counts"),
            ("Affected ask-*.skill", "Update data count in skill header/instructions"),
        ],
    },
    "task_completed": {
    ...
    "learning_captured": {
        ...
        "updates": [
            ...
            ("self-improve.skill", "Update if it changes audit criteria or red flags"),
        ],
    },
    "prd_written": {
...
def get_disk_skills():
    """Get all .skill files on disk."""
    if not SKILLS_DIR.exists():
        return set()
    return {f.stem for f in SKILLS_DIR.glob("*.skill") if f.is_file()}
...
def check_skill_consistency(disk_skills, registry_names, registry_by_name, actual_counts):
    ...
    for skill in sorted(missing_from_disk):
        issues.append(Issue(
            ...
            f"Skill '{skill}' in registry but .skill file MISSING from disk",
        ))
