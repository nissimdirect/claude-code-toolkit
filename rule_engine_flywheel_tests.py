...
from rule_engine import (
    Rule, load_rules, load_state, save_state, apply_state, extract_state,
    classify_prompt, score_rule, get_relevant_rules, record_violation,
    advance_day, detect_inactive, detect_merge_candidates, get_lifecycle_report,
    validate_rules, clear_cache,
    THRESHOLD, BUDGET, INJECTION_LIMIT, SPIKE_CAP, IMMUNE_SPIKE,
    SPIKE_DECAY_PER_DAY, INACTIVE_THRESHOLD_DAYS, SPIKE_AMOUNT,
    DOMAIN_SCORE_EXPLICIT, DOMAIN_SCORE_UNIVERSAL, SPREAD_BONUS_FACTOR,
    CO_ACTIVATION_MAX_PAIRS, CO_ACTIVATION_MIN_EITHER,
...
PRINCIPLES_PATH = Path.home() / ".claude/projects/-Users-nissimagent/memory/behavioral-principles.md"
...
def test_phase1_violation_spike(runner: TestRunner):
    ...
    try:
        ...
        rules = load_rules()
        clean_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                       "days_since_activation": 0, "activation_count": 0}
                       for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                       "last_decay": "", "updated_at": ""}
        ...
        runner.record(phase, "Hook violation adds spike to R4",
        ...
        runner.record(phase, "User violation adds correct spike",
        ...
        runner.record(phase, "Self-check violation adds correct spike",
        ...
        runner.record(phase, "Audit violation adds correct spike",
        ...
        state = load_state()
        spike = state["rules"]["R4"]["adaptive_spike"]
        runner.record(phase, "Spike capped at SPIKE_CAP after many violations",
                      spike == SPIKE_CAP,
        ...
        runner.record(phase, "Nonexistent rule violation returns None",
                      result is None)
        ...
        state = load_state()
        runner.record(phase, "State file updated_at reflects violation time",
        ...
        runner.record(phase, "Three rules have independent spikes",
...
def test_phase2_spike_injection(runner: TestRunner):
    ...
    try:
        ...
        runner.record(phase, "Spiked rule scores higher than unspiked",
        ...
        runner.record(phase, "Spike contribution equals SPIKE_CAP",
        ...
        runner.record(phase, "Spiked R42 appears in audio query results",
                      "R42" in result_ids,
        ...
        if results:
            ...
            runner.record(phase, "All results include numeric score",
                          has_score)
        else:
            runner.record(phase, "All results include numeric score", False,
                          "No results returned")

        # Test 5: Result includes domain classification for visibility
        if results:
            has_domain = all("domain" in r for r in results)
            runner.record(phase, "All results include domain classification",
                          has_domain)
        else:
        ...
        if results:
            ...
            runner.record(phase, "All results include tool classification",
                          has_tool)
        else:
        ...
        if results:
            ...
            runner.record(phase, "All results include principle_id",
                          has_pid)
        else:
        ...
        runner.record(phase, f"Results capped at INJECTION_LIMIT={INJECTION_LIMIT}",
        ...
        if results:
            ...
            runner.record(phase, "All results include reason tag",
                          has_reason,
                          f"reasons={[r.get('reason') for r in results]}")
        else:
            runner.record(phase, "All results include reason tag", False,
                          "No results returned")
        ...
        if r42_result:
            runner.record(phase, "Spiked rule reason contains 'spiked'",
                          "spiked" in r42_result.get("reason", ""),
                          f"reason={r42_result.get('reason')}")
        else:
            runner.record(phase, "Spiked rule reason contains 'spiked'", False,
                          "R42 not in results")
...
def test_phase3_activation_tracking(runner: TestRunner):
    ...
    try:
        rules = load_rules()
        clean_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                       "days_since_activation": 5, "activation_count": 0}
                       for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                       "last_decay": "", "updated_at": ""}
        save_state(clean_state)
        ...
        state = load_state()
        activated_rules = [rid for rid, rs in state["rules"].items()
                          if rs["activation_count"] > 0]
        runner.record(phase, "At least one rule activated after query",
                      len(activated_rules) > 0,
                      f"activated: {activated_rules}")

        # Test 2: days_since_activation resets for activated rules
        activated_reset = [rid for rid, rs in state["rules"].items()
                          if rs["activation_count"] > 0 and rs["days_since_activation"] == 0]
        runner.record(phase, "Activated rules have days_since_activation=0",
                      len(activated_reset) > 0,
                      f"reset: {activated_reset}")

        # Test 3: Non-activated rules keep their days_since_activation
        non_activated = [rid for rid, rs in state["rules"].items()
                        if rs["activation_count"] == 0]
        kept_days = all(state["rules"][rid]["days_since_activation"] == 5
                       for rid in non_activated if rid in state["rules"])
        runner.record(phase, "Non-activated rules keep days_since_activation",
        ...
        runner.record(phase, "Co-activation dict is populated",
                      len(co_act) > 0,
                      f"pairs: {len(co_act)}")
        ...
        if co_act:
            ...
            runner.record(phase, "Co-activation pairs have both/either counts",
                          has_counts,
                          f"sample: {co_act[first_key]}")
        else:
            runner.record(phase, "Co-activation pairs have both/either counts",
                          False, "no co-activation data")
        ...
        runner.record(phase, "Repeated queries increment activation count",
                      max_count >= 3,
        ...
        runner.record(phase, "State persists across reloads",
...
def test_phase4_decay(runner: TestRunner):
    ...
    try:
        ...
        spiked_state = {"rules": {rid: {"adaptive_spike": SPIKE_CAP if rid == "R4" else 0.0,
                        "dormant": False, "days_since_activation": 0, "activation_count": 5}
                        for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                        "last_decay": "", "updated_at": ""}
        save_state(spiked_state)
        advance_day()
        ...
        runner.record(phase, "advance_day reduces spike by SPIKE_DECAY_PER_DAY",
                      abs(new_spike - expected) < 0.001,
                      f"new={new_spike}, expected={expected}")

        # Test 2: Spike decays to zero (not negative)
        tiny_state = {"rules": {rid: {"adaptive_spike": 0.005 if rid == "R4" else 0.0,
                      "dormant": False, "days_since_activation": 0, "activation_count": 5}
                      for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                      "last_decay": "", "updated_at": ""}
        ...
        state = load_state()
        spike = state["rules"]["R4"]["adaptive_spike"]
        runner.record(phase, "Spike decays to 0.0, never negative",
                      spike == 0.0,
                      f"spike={spike}")
        ...
        runner.record(phase, "days_since_activation increments by 1",
                      days == 1,
                      f"days={days}")

        # Test 4: Multiple advance_day calls compound correctly
        fresh_state = {"rules": {rid: {"adaptive_spike": SPIKE_CAP if rid == "R9" else 0.0,
                       "dormant": False, "days_since_activation": 0, "activation_count": 5}
                       for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                       "last_decay": "", "updated_at": ""}
        ...
        runner.record(phase, f"Spike reaches 0 after {days_to_zero} days",
                      spike_final == 0.0,
                      f"spike={spike_final}")
        ...
        state = load_state()
        r9_spike = state["rules"]["R9"]["adaptive_spike"]
        runner.record(phase, "Unspiked rules stay at 0 through decay",
                      r9_spike == 0.0)
...
def test_phase5_dormancy(runner: TestRunner):
    ...
    try:
        ...
        dormant_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
        ...
        runner.record(phase, "R42 at 65 days detected as inactive",
                      "R42" in candidates,
                      f"candidates={candidates}")

        # Test 2: Pinned rules are NOT detected as dormancy candidates
        pinned_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                        "days_since_activation": 100, "activation_count": 0}
                        for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                        "last_decay": "", "updated_at": ""}
        ...
        runner.record(phase, "Pinned rules never become dormancy candidates",
                      len(pinned_in_candidates) == 0,
                      f"pinned_in_candidates={pinned_in_candidates}")

        # Test 3: Already dormant rules not re-detected
        already_dormant_state = {"rules": {rid: {"adaptive_spike": 0.0,
        ...
        candidates = detect_inactive()
        runner.record(phase, "Already dormant rules not re-detected",
                      "R42" not in candidates)
        ...
        runner.record(phase, "Dormant rule scores 0.0",
                      dormant_score == 0.0,
        ...
        result_ids = [r["id"] for r in results]
        runner.record(phase, "Dormant R42 excluded from query results",
                      "R42" not in result_ids,
                      f"result_ids={result_ids}")

        # Test 6: Rule at exactly 59 days NOT detected
        edge_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
        ...
        candidates = detect_inactive()
        runner.record(phase, "Rule at 59 days NOT detected as inactive",
                      "R67" not in candidates)

        # Test 7: Rule at exactly 60 days IS detected
        boundary_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
        ...
        candidates = detect_inactive()
        runner.record(phase, "Rule at exactly 60 days IS detected",
                      "R67" in candidates)
...
def test_phase6_immune(runner: TestRunner):
    ...
    try:
        ...
        dormant_state = {"rules": {rid: {"adaptive_spike": 0.0,
                         "dormant": True if rid == "R42" else False,
        ...
        state = load_state()
        runner.record(phase, "Dormant R42 reactivated by violation",
        ...
        runner.record(phase, "Immune spike set to IMMUNE_SPIKE",
                      spike == IMMUNE_SPIKE,
                      f"spike={spike}, expected={IMMUNE_SPIKE}")

        # Test 3: Reactivation returns a message (visibility)
        runner.record(phase, "Reactivation returns visible message",
        ...
        result_ids = [r["id"] for r in results]
        runner.record(phase, "Reactivated R42 appears in audio queries",
                      "R42" in result_ids,
                      f"result_ids={result_ids}")

        # Test 5: IMMUNE_SPIKE > SPIKE_CAP (stronger comeback)
        runner.record(phase, "IMMUNE_SPIKE exceeds normal SPIKE_CAP",
        ...
        save_state(dormant_state)
        save_state({"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                    "days_since_activation": 0, "activation_count": 5}
                    for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                    "last_decay": "", "updated_at": ""})
        msg_normal = record_violation("R4", "hook")
        runner.record(phase, "Normal violation returns None (not immune msg)",
                      msg_normal is None)
...
def test_phase7_merge(runner: TestRunner):
    ...
    try:
        ...
        co_act_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                        "days_since_activation": 0, "activation_count": 0}
                        for rid in rules},
                        "co_activation": {
        ...
        pair_found = any((r1 == "R27" and r2 == "R4") or (r1 == "R4" and r2 == "R27")
                        for r1, r2, _ in merges)
        runner.record(phase, "100% co-activation pair detected as merge candidate",
                      pair_found,
                      f"merges={merges}")

        # Test 2: Low co-activation NOT flagged
        low_co_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                        "days_since_activation": 0, "activation_count": 0}
                        for rid in rules},
                        "co_activation": {
        ...
        merges = detect_merge_candidates()
        runner.record(phase, "50% co-activation NOT flagged as merge",
                      len(merges) == 0)

        # Test 3: Insufficient data (either < 10) NOT flagged
        few_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                     "days_since_activation": 0, "activation_count": 0}
                     for rid in rules},
                     "co_activation": {
        ...
        merges = detect_merge_candidates()
        runner.record(phase, "High ratio but <10 observations NOT flagged",
                      len(merges) == 0)

        # Test 4: Recently unmerged pair excluded
        unmerge_state = {"rules": {rid: {"adaptive_spike": 0.0, "dormant": False,
                         "days_since_activation": 0, "activation_count": 0}
                         for rid in rules},
                         "co_activation": {
        ...
        merges = detect_merge_candidates()
        runner.record(phase, "Recently unmerged pair excluded from merge",
                      len(merges) == 0)
        ...
        if merges:
            ...
            runner.record(phase, "Merge candidate includes ratio",
                          ratio >= 0.9,
                          f"ratio={ratio}")
        else:
            runner.record(phase, "Merge candidate includes ratio", False,
                          "no candidates")
...
def test_phase8_lifecycle_report(runner: TestRunner):
    ...
    try:
        ...
        rich_state = {"rules": {}, "co_activation": {
            "R27:R4": {"both": 12, "either": 12},
        }, "recently_unmerged": {},
        "last_decay": "2026-02-14", "updated_at": ""}
        ...
        runner.record(phase, "Report includes inactive_candidates",
                      "inactive_candidates" in report)

        # Test 6: Report has merge candidates
        runner.record(phase, "Report includes merge_candidates",
                      "merge_candidates" in report)

        # Test 7: Report has last_decay date (visibility of maintenance)
        runner.record(phase, "Report includes last_decay date",
...
def test_phase9_hook_integration(runner: TestRunner):
    ...
    runner.record(phase, "learning_hook.py exists",
                  HOOK_PATH.exists(),
                  f"path={HOOK_PATH}")
    ...
    if HOOK_PATH.exists():
        ...
        runner.record(phase, "Hook imports rule_engine",
                      "import rule_engine" in hook_source or "rule_engine" in hook_source)

        # Test 3: Hook has graceful degradation (try/except)
        runner.record(phase, "Hook has graceful degradation",
                      "except" in hook_source and "rule_engine" in hook_source)

        # Test 4: Hook produces additionalContext
        runner.record(phase, "Hook outputs additionalContext",
                      "additionalContext" in hook_source)
    else:
        for name in ["Hook imports rule_engine", "Hook has graceful degradation",
    ...
    try:
        event = json.dumps({"event": "UserPromptSubmit",
                           "data": {"message": "edit the python function to fix the bug"}})
        result = subprocess.run(
            [sys.executable, str(HOOK_PATH)],
            input=event, capture_output=True, text=True, timeout=5
        )
        output = result.stdout.strip()
        parsed = json.loads(output) if output else {}
        runner.record(phase, "Hook runs successfully on code prompt",
                      result.returncode == 0,
        ...
        runner.record(phase, "Hook returns rule injections in context",
                      has_rules,
    ...
    try:
        ...
        parsed = json.loads(output) if output else {}
        context = parsed.get("additionalContext", "")
        runner.record(phase, "Short prompt 'ok' produces no rule injection",
                      "[Rule " not in context,
                      f"context={context[:80]}")
    except Exception as e:
        runner.record(phase, "Short prompt 'ok' produces no rule injection", False, str(e))
    ...
    try:
        ...
        runner.record(phase, "Git prompt returns git-relevant rules",
                      has_git_rule,
                      f"context_preview={context[:120]}")
    except Exception as e:
    ...
    try:
        ...
        runner.record(phase, "Hook injection includes reason tag in format",
                      has_reason_pipe,
                      f"context_preview={context[:150]}")
    except Exception as e:
        runner.record(phase, "Hook injection includes reason tag in format", False, str(e))
...
def test_phase10_full_flywheel(runner: TestRunner):
    ...
    try:
        ...
        runner.record(phase, "Step 1: R42 baseline score captured",
                      True, f"score={score_before}")

        # STEP 2: Violation occurs — spike added
        record_violation("R42", "hook")
        state = load_state()
        spike_after_violation = state["rules"]["R42"]["adaptive_spike"]
        runner.record(phase, "Step 2: Violation adds spike",
        ...
        runner.record(phase, "Step 3: Spiked rule scores higher",
        ...
        runner.record(phase, "Step 4: Spike decays over 15 days",
        ...
        save_state(state)
        candidates = detect_inactive()
        runner.record(phase, "Step 5: Long-inactive rule is dormancy candidate",
                      "R42" in candidates)
        ...
        runner.record(phase, "Step 6: Dormant rule excluded from results",
                      "R42" not in dormant_ids)

        # STEP 7: Antigen recurs — immune reactivation!
        msg = record_violation("R42", "user")
        state = load_state()
        runner.record(phase, "Step 7: Immune reactivation on violation",
                      state["rules"]["R42"]["dormant"] is False and
                      state["rules"]["R42"]["adaptive_spike"] == IMMUNE_SPIKE,
        ...
        r42_reactivated = next((r for r in results_reactivated if r["id"] == "R42"), None)
        runner.record(phase, "Step 8: Reactivated rule appears in results",
        ...
        report = get_lifecycle_report()
        runner.record(phase, "Step 9: Lifecycle report reflects current state",
                      report["total_rules"] > 0 and "spiked_rules" in report)

        # STEP 10: The flywheel is complete — all stages verified
        runner.record(phase, "Step 10: FLYWHEEL COMPLETE",
                      True, "violation→spike→boost→decay→dormancy→immune→reactivation")
...
def test_phase11_classification(runner: TestRunner):
    ...
    for expected_domain, prompt in domain_prompts.items():
        ...
        runner.record(phase, f"'{expected_domain}' domain classified correctly",
    ...
    for expected_tool, prompt in tool_prompts.items():
        domain, tool = classify_prompt(prompt)
        runner.record(phase, f"'{expected_tool}' tool classified correctly",
                      tool == expected_tool,
                      f"got tool='{tool}' from '{prompt[:40]}...'")

    # Test stem matching
    domain, tool = classify_prompt("editing some files and refactoring")
    runner.record(phase, "Stem matching: 'editing' → edit tool",
                  tool == "edit",
                  f"tool={tool}")

    domain, tool = classify_prompt("compiling the code and running tests")
    runner.record(phase, "Stem matching: 'compiling' → bash tool",
                  tool == "bash",
                  f"tool={tool}")
...
def test_phase12_spreading(runner: TestRunner):
    ...
    try:
        ...
        if r4:
            ...
            runner.record(phase, "R4 has connection to R9",
                          has_r9_connection,
        ...
        runner.record(phase, "Connection graph has edges",
        ...
        runner.record(phase, "All connection targets exist as rules",
                      len(missing_targets) == 0,
                      f"missing={missing_targets}")

        # Test 4: Connection weights are between 0 and 1
        bad_weights = [(rid, cid, w) for rid, cid, w in connection_pairs
                      if w < 0 or w > 1]
        runner.record(phase, "All connection weights in [0, 1]",
                      len(bad_weights) == 0,
        ...
        runner.record(phase, "Spreading bonus formula verified",
                      expected_bonus == 0.12,
...
def test_phase13_performance(runner: TestRunner):
    ...
    try:
        ...
        runner.record(phase, "Cold query < 50ms",
                      cold_ms < 50,
        ...
        runner.record(phase, "Cached query avg < 5ms (10 iterations)",
                      avg_ms < 5,
                      f"avg={avg_ms:.2f}ms")
        ...
        runner.record(phase, "record_violation < 10ms",
                      violation_ms < 10,
        ...
        runner.record(phase, "advance_day < 10ms",
                      advance_ms < 10,
                      f"advance={advance_ms:.1f}ms")

        # Test 5: lifecycle report < 20ms
        start = time.perf_counter()
        get_lifecycle_report()
        report_ms = (time.perf_counter() - start) * 1000
        runner.record(phase, "lifecycle_report < 20ms",
                      report_ms < 20,
        ...
        runner.record(phase, "Hook end-to-end < 5000ms",
        ...
        runner.record(phase, "validate_rules < 5ms",
                      validate_ms < 5,
...
def test_phase14_malformed(runner: TestRunner):
    ...
    try:
        ...
        runner.record(phase, "Empty prompt returns list without crash",
        ...
        try:
            ...
            runner.record(phase, "Unicode prompt doesn't crash",
                          True)
        except Exception as e:
        ...
        try:
            ...
            runner.record(phase, "10K-word prompt doesn't crash",
                          isinstance(results, list))
        except Exception as e:
        ...
        try:
            ...
            state = load_state()
            runner.record(phase, "Corrupted JSON returns empty dict",
                          state == {},
        ...
        try:
            ...
            r4 = fresh_rules["R4"]
            runner.record(phase, "Bad types in state clamped safely",
                          r4.adaptive_spike == 0.0 and
                          r4.days_since_activation == 0 and
                          r4.activation_count == 0,
        ...
        r4 = fresh_rules2["R4"]
        runner.record(phase, "Negative values clamped to 0",
                      r4.adaptive_spike == 0.0 and
                      r4.days_since_activation == 0 and
                      r4.activation_count == 0)

        # Test 7: State with spike > IMMUNE_SPIKE — clamped
        overflow_state = {"rules": {"R4": {
        ...
        runner.record(phase, "Spike > IMMUNE_SPIKE clamped to IMMUNE_SPIKE",
        ...
        bad_rules = {
            "BAD1": Rule(id="BAD1", principle_id="P999", name="Bad",
                         tier="legendary", pinned=False, domains=["mars"],
                         consequence=5.0, tool_triggers=[], antigens=[],
                         connections={"NONEXISTENT": 2.5}, reminder="test"),
        }
        warnings = validate_rules(bad_rules)
        runner.record(phase, "Schema validates bad tier, domain, consequence, connection",
                      len(warnings) >= 4,
                      f"warnings={len(warnings)}: {warnings[:2]}")

        # Test 9: classify_prompt with only special chars
        domain, tool = classify_prompt("!@#$%^&*(){}[]")
        runner.record(phase, "Special chars prompt returns defaults",
        ...
        try:
            ...
            runner.record(phase, "Missing state file — engine still returns results",
                          isinstance(results, list),
                          f"got {len(results)} results")
        finally:
        ...
        rules = load_rules()
        ancient_state = {"rules": {rid: {"adaptive_spike": SPIKE_CAP if rid == "R4" else 0.0,
                         "dormant": False, "days_since_activation": 0, "activation_count": 5}
                         for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                         "last_decay": "2020-01-01", "updated_at": ""}
        ...
        runner.record(phase, f"Ancient last_decay capped at {CALENDAR_DECAY_MAX_DAYS} days",
                      max_days <= CALENDAR_DECAY_MAX_DAYS,
                      f"max_days_since_activation={max_days}")

        # Test 12: Future last_decay date treated as 1 day
        future_state = {"rules": {rid: {"adaptive_spike": SPIKE_CAP if rid == "R4" else 0.0,
                        "dormant": False, "days_since_activation": 0, "activation_count": 5}
                        for rid in rules}, "co_activation": {}, "recently_unmerged": {},
                        "last_decay": "2099-12-31", "updated_at": ""}
        save_state(future_state)
        advance_day()
        state = load_state()
        days_after = state["rules"]["R4"]["days_since_activation"]
        runner.record(phase, "Future last_decay treated as 1 day (not negative)",
                      days_after == 1,
                      f"days={days_after}")

        # Test 13: No fcntl import in rule_engine (dead import removed)
        import inspect
        source = inspect.getsource(rule_engine)
        runner.record(phase, "No dead fcntl import in rule_engine.py",
                      "import fcntl" not in source,
                      "fcntl removed — no false sense of file locking")
...
def test_phase15_concurrent(runner: TestRunner):
    ...
    try:
        ...
        runner.record(phase, "5 concurrent violations don't crash",
                      len(errors) == 0,
        ...
        try:
            ...
            valid_json = False
        runner.record(phase, "State file valid JSON after concurrent writes",
                      valid_json)
        ...
        runner.record(phase, "5 concurrent queries don't crash",
                      len(query_errors) == 0,
        ...
        state = load_state()
        runner.record(phase, "Mixed concurrent reads/writes don't corrupt state",
        ...
        runner.record(phase, "50 concurrent save_state calls don't corrupt",
...
def main():
    ...
    print(f"  P1: Violation→Spike ... {runner.phase_counts.get('P1: Violation→Spike', (0,0))}")

    test_phase2_spike_injection(runner)
    print(f"  P2: Spike→Injection ... {runner.phase_counts.get('P2: Spike→Injection', (0,0))}")

    test_phase3_activation_tracking(runner)
    print(f"  P3: Activation Tracking ... {runner.phase_counts.get('P3: Activation Tracking', (0,0))}")
    ...
    print(f"  P6: Immune Reactivation ... {runner.phase_counts.get('P6: Immune Reactivation', (0,0))}")

    test_phase7_merge(runner)
    print(f"  P7: Merge Detection ... {runner.phase_counts.get('P7: Merge Detection', (0,0))}")

    test_phase8_lifecycle_report(runner)
    print(f"  P8: Lifecycle Report ... {runner.phase_counts.get('P8: Lifecycle Report', (0,0))}")

    test_phase9_hook_integration(runner)
    print(f"  P9: Hook Integration ... {runner.phase_counts.get('P9: Hook Integration', (0,0))}")

    test_phase10_full_flywheel(runner)
    print(f"  P10: Full Flywheel ... {runner.phase_counts.get('P10: Full Flywheel', (0,0))}")

    test_phase11_classification(runner)
    print(f"  P11: Classification Visibility ... {runner.phase_counts.get('P11: Classification Visibility', (0,0))}")

    test_phase12_spreading(runner)
    print(f"  P12: Spreading Activation ... {runner.phase_counts.get('P12: Spreading Activation', (0,0))}")

    test_phase13_performance(runner)
    print(f"  P13: Performance ... {runner.phase_counts.get('P13: Performance', (0,0))}")

    test_phase14_malformed(runner)
    print(f"  P14: Malformed Input ... {runner.phase_counts.get('P14: Malformed Input', (0,0))}")

    test_phase15_concurrent(runner)
    print(f"  P15: Concurrent Access ... {runner.phase_counts.get('P15: Concurrent Access', (0,0))}")
