...
def phase1_pre_migration_snapshot():
    ...
    results.record(
        phase,
        "behavioral-principles.md exists",
        bp_exists,
    ...
    if bp_exists:
        ...
        results.record(
            phase,
            f"Core principles counted ({core_count})",
            core_count >= 30,
            f"expected >=30, got {core_count}",
        )
        ...
        results.record(
            phase,
            f"Meta-directives counted ({md_count})",
            md_count == 7,
            f"expected 7, got {md_count}",
        )
        ...
        results.record(
            phase,
            f"Domain principles counted ({domain_count})",
            domain_count >= 15,
            f"expected >=15, got {domain_count}",
        )
        ...
        results.record(
            phase,
            f"Cross-reference rows ({xref_count})",
            xref_count >= 50,
            f"expected >=50, got {xref_count}",
        )

        # Check for immutable user quotes and mistake count from learning-index.json
        index_path = Path.home() / ".claude/.locks/learning-index.json"
        if index_path.exists():
            _idx = json.loads(index_path.read_text())
            quote_count = len(_idx.get("quotes", []))
            results.record(
                phase,
                f"Immutable user quotes ({quote_count})",
                quote_count >= 10,
                f"expected >=10, got {quote_count}",
            )

            mistake_count = _idx.get("total_learnings", 0)
            results.record(
                phase,
                f"Numbered mistakes ({mistake_count})",
                mistake_count >= 30,
                f"expected >=30, got {mistake_count}",
            )
        else:
            results.skip(phase, "Learnings index check", "index not found")

    # Check coverage_matrix.py exists
    cm_path = Path.home() / "Development/tools/coverage_matrix.py"
    results.record(
        phase,
        "coverage_matrix.py exists",
        cm_path.exists(),
    ...
    if idx_path.exists():
        ...
        results.record(
            phase,
            f"Learning index entries ({entry_count})",
            entry_count >= 50,
            f"expected >=50, got {entry_count}",
        )
    else:
...
def phase2_migration_correctness(bp_content):
    ...
    results.record(
        phase,
        f"Total unique principle IDs extracted ({len(all_ids)})",
        len(all_ids) >= 40,
        f"expected >=40, got {len(all_ids)}",
    )
    ...
    for r_id, p_id in seed_to_principle_map.items():
        ...
        results.record(
            phase,
            f"Seed {r_id} → Principle {p_id} exists",
            p_exists,
            f"{p_id} not found in behavioral-principles.md",
        )
    ...
    results.record(
        phase,
        f"Cross-reference mistake numbers ({len(xref_mistake_nums)})",
    ...
    results.record(
        phase,
        f"No empty principles (found {len(empty_principles)} empty)",
...
def phase3_scoring_correctness():
    ...
    tests = [
        ...
        (
            ...
            {"R42"},
            {"R30", "R84"},
            "audio+read → Audio Terms, not git/locations",
        ),
        (
            "writing",
            "write",
            {"R6", "R70"},
            {"R42", "R30"},
            "writing+write → Ideas/PRD, not audio/git",
        ),
        (
            "writing",
            "skill",
            {"R6", "R70"},
            {"R42"},
            "writing+skill → Ideas/PRD, not audio",
        ),
        (
            "git",
            "bash",
            {"R30", "R50"},
            {"R42", "R70"},
            "git+bash → Git/Security, not audio/PRD",
        ),
        (
            "advisory",
            "skill",
            {"R6"},
            {"R42", "R30"},
            "advisory+skill → Ideas, not audio/git",
        ),
        (
            "budget",
            "bash",
            {"R75"},
            {"R42", "R70"},
            "budget+bash → Budget, not audio/PRD",
        ),
        (
            "scraping",
            "bash",
            {"R8", "R86"},
            {"R42", "R70"},
            "scraping+bash → Systems/FS, not audio/PRD",
        ),
    ]
    ...
    for spike_id, spike_domain, test_domain in [
        ...
        activated = eng.retrieve(Prompt(test_domain, "read", "spike leak test"))
        activated_ids = {r.id for r, _ in activated}
        passed = spike_id not in activated_ids
        results.record(
            phase,
            f"Spike containment: {spike_id} spiked, not in {test_domain}",
            passed,
            f"{spike_id} leaked to {test_domain}" if not passed else "",
        )
    ...
    if len(activated) >= 2:
        ...
        results.record(
            phase,
            f"Budget #1 is highest-scoring ({top_id}={top_score:.2f})",
            top_score >= 0.8,
            f"top was {top_id}={top_score:.2f}",
        )
    ...
    results.record(
        phase,
        "R99 (Practice) excluded by budget in code+edit",
    ...
    results.record(
        phase,
        f"Max possible score = {max_score:.2f} (bounded, documented)",
        max_score <= 1.10,
        f"unexpectedly high: {max_score}",
    )

    # Universal max: 0.3 (all domain) + 0.3 + 0.2 + 0.15 = 0.95
    max_rule_univ = Rule("MAXU", "max univ", "value", True, ["all"], 0.3, ["bash"], [])
    max_rule_univ.adaptive_spike = 0.15
    max_univ = eng_bounds.score(max_rule_univ, Prompt("code", "bash", "max test"))
    results.record(
        phase,
        f"Max universal score = {max_univ:.2f} (< explicit max)",
        max_univ < max_score,
        f"universal={max_univ}, explicit={max_score}",
    )
    ...
    results.record(
        phase,
        "Tiebreaker: explicit domain beats universal",
    ...
    results.record(
        phase,
        "Tiebreaker at equal score: explicit domain wins",
...
def phase4_lifecycle():
    ...
    results.record(
        phase,
        f"Spike capped at {eng.SPIKE_CAP}",
    ...
    results.record(
        phase,
        "Spike decays after advance_day",
    ...
    results.record(
        phase,
        "Spike fully decays to 0 after 20 days",
    ...
    results.record(
        phase,
        "R42 detected as inactive at 60 days",
        "R42" in inactive_ids,
    ...
    results.record(
        phase, "Pinned R4 NOT inactive even at 100 days", "R4" not in inactive_ids2
    )
    ...
    results.record(phase, "Dormant rule scores 0.0", score == 0.0, f"score={score}")
    ...
    results.record(
        phase, "Immune reactivation clears dormant flag", not eng5.rules["R42"].dormant
    )
    results.record(
        phase,
        f"Immune spike = {eng5.IMMUNE_SPIKE}",
        eng5.rules["R42"].adaptive_spike == eng5.IMMUNE_SPIKE,
        f"spike={eng5.rules['R42'].adaptive_spike}",
    )
    results.record(
        phase,
        "Immune reactivation returns message",
        result is not None and "IMMUNE" in result,
        f"result={result}",
    )
    ...
    results.record(
        phase,
        "Co-activation >90% detected as merge candidate",
    ...
    results.record(
        phase,
        "Un-merge cooldown blocks re-merge detection",
    ...
    results.record(
        phase,
        "Un-merge cooldown expires after 30 days",
    ...
    results.record(
        phase,
        "Spike never goes negative after 100 decays",
        spike_val >= 0,
        f"spike={spike_val}",
    )
    results.record(
        phase, "Decayed spike is exactly 0.0", spike_val == 0.0, f"spike={spike_val}"
    )
    ...
    results.record(
        phase,
        "Spike at 0.001 snaps to 0 after decay",
    ...
    results.record(
        phase,
        "Spike increases rule's activation score",
    ...
    spike_activated = eng_spike_list.retrieve(
        Prompt("audio", "edit", "spike list test")
    )
    spike_ids = {r.id for r, _ in spike_activated}
    results.record(
        phase,
        "Spiked R42 appears in activation list",
        "R42" in spike_ids,
        f"activated={spike_ids}",
    )
    ...
    results.record(
        phase, "Zero rules → empty activation (no crash)", len(act_zero) == 0
    )
    ...
    results.record(
        phase,
        "One rule activates if eligible",
    ...
    results.record(
        phase,
        f"100 rules respects budget={eng_hundred.BUDGET}",
    ...
    results.record(
        phase,
        "All dormant except R4 → only R4 activates",
    ...
    results.record(
        phase,
        "Dormant+pinned rule scores 0 (dormant takes precedence in scoring)",
        conflict_score == 0.0,
        f"score={conflict_score}",
    )
    ...
    results.record(
        phase, "R4 activates in code+edit (spreading source)", "R4" in activated_ids
    )
...
def phase5_integration():
    ...
    if cm_path.exists():
        ...
        results.record(
            phase,
            "coverage_matrix.py gate passes",
            gate_passed,
    ...
    if lh_path.exists():
        ...
        result = subprocess.run(
            [
                "python3",
                "-c",
                f"import py_compile; py_compile.compile('{lh_path}', doraise=True)",
            ],
            capture_output=True,
            text=True,
            timeout=10,
    ...
    if lc_path.exists():
        ...
        result = subprocess.run(
            [
                "python3",
                "-c",
                f"import py_compile; py_compile.compile('{lc_path}', doraise=True)",
            ],
            capture_output=True,
            text=True,
            timeout=10,
    ...
    result = subprocess.run(
        [
            "python3",
            "-c",
            f"import py_compile; py_compile.compile('{sim_path}', doraise=True)",
        ],
        capture_output=True,
        text=True,
        timeout=10,
    ...
    results.record(
        phase, "DOMAIN_SCORE_EXPLICIT = 0.4", eng.DOMAIN_SCORE_EXPLICIT == 0.4
    )
    results.record(
        phase, "DOMAIN_SCORE_UNIVERSAL = 0.3", eng.DOMAIN_SCORE_UNIVERSAL == 0.3
    )
...
def phase6_red_team_fixes():
    ...
    results.record(
        phase,
        "Unknown domain excludes all domain-specific rules",
    ...
    for rid in critical_ids:
        results.record(
            phase,
            f"{rid} is pinned (can't go dormant)",
    ...
    for rid in critical_ids:
        results.record(
            phase, f"{rid} NOT inactive at 200 days (pinned)", rid not in inactive_ids
        )
    ...
    results.record(
        phase,
        "10 violations still capped at SPIKE_CAP",
    ...
    results.record(
        phase,
        "All rules dormant → empty activation",
        len(activated) == 0,
    ...
    audio_only_rules = {
        r.id for r in eng_spoof.rules.values() if r.domains == ["audio"]
    }
    leaked = audio_only_rules & spoof_ids
    results.record(
        phase,
        "Domain='all' doesn't leak audio-only rules",
        len(leaked) == 0,
    ...
    domain_specific = {r.id for r in eng_spoof.rules.values() if "all" not in r.domains}
    leaked_empty = domain_specific & spoof_empty_ids
    results.record(
        phase,
        "Empty domain excludes all domain-specific rules",
        len(leaked_empty) == 0,
    ...
    results.record(
        phase,
        "Domain='None' excludes domain-specific rules",
        len(leaked_none) == 0,
    ...
    results.record(
        phase,
        "Spike trajectory is monotonically non-decreasing",
        monotonic,
        f"trajectory={[f'{s:.3f}' for s in trajectory]}",
    )

    # Cap should be hit by violation 1-2 (hook=0.15, cap=0.15)
    cap_hit_at = next(
        (i for i, s in enumerate(trajectory) if s >= eng_stack.SPIKE_CAP), None
    )
    results.record(
        phase,
        f"Spike cap hit by violation #{cap_hit_at + 1 if cap_hit_at is not None else 'never'}",
    ...
    results.record(
        phase,
        "All post-cap violations stay at SPIKE_CAP",
        all(s == eng_stack.SPIKE_CAP for s in trajectory[1:]),
        "post-cap values vary"
        if not all(s == eng_stack.SPIKE_CAP for s in trajectory[1:])
        else "",
    )
    ...
    results.record(
        phase,
        "High-value R4 survives budget starvation",
        "R4" in starve_ids,
        f"R4 not in {starve_ids}",
    )

    cheap_count = sum(1 for rid in starve_ids if rid.startswith("CHEAP"))
    results.record(
        phase,
        f"Budget not fully starved by cheap rules ({cheap_count}/5 slots)",
    ...
    results.record(
        phase,
        "High-consequence R50 (0.3) beats cheap universals in code+bash",
        "R50" in starve_ids3,
    ...
    results.record(
        phase,
        "No dual-domain rules (explicit + 'all')",
...
def phase7_rollback():
    ...
    results.record(
        phase,
        "behavioral-principles.md has git history",
        has_git_history,
        result.stderr[:200]
        if not has_git_history
        else f"commits: {result.stdout.strip()[:100]}",
    )

    # learning_hook.py is in hooks dir — verify backup approach
    lh_path = Path.home() / ".claude/hooks/learning_hook.py"
    results.record(phase, "learning_hook.py exists (rollback target)", lh_path.exists())
    ...
    results.record(
        phase,
        "Sim has no production file imports",
    ...
    results.record(
        phase,
        "Engine has no file I/O operations",
        len(found_io) == 0,
    ...
    results.record(
        phase,
        f"behavioral-principles.md has {commit_count} recent commits",
        commit_count >= 2,
    ...
    results.record(
        phase,
        "behavioral-principles.md has no uncommitted changes",
        bp_clean,
...
def phase8_information_loss():
    ...
    for concept in key_concepts:
        ...
        results.record(
            phase,
            f"Seed rules cover '{concept}'",
            found,
            "not found in seed rule names" if not found else "",
        )
    ...
    results.record(
        phase,
        f"Values: {tiers['value']} (expected 5)",
        tiers["value"] == 5,
        f"got {tiers['value']}",
    )
    results.record(
        phase,
        f"Principles: {tiers['principle']} (expected >=10)",
        tiers["principle"] >= 10,
        f"got {tiers['principle']}",
    )
    results.record(
        phase,
        f"Practices: {tiers['practice']} (expected >=3)",
        tiers["practice"] >= 3,
        f"got {tiers['practice']}",
    )
    ...
    results.record(
        phase,
        "All 8 domains covered in seed rules",
    ...
    results.record(
        phase,
        f"Connection graph: {len(connected_rules)} connected, {len(isolated)} isolated",
    ...
    results.record(
        phase,
        f"Antigens mapped ({antigen_count} total)",
        antigen_count >= 10,
        f"got {antigen_count}",
    )
    ...
    for pid, keywords in content_checks.items():
        if pid in principle_bodies:
            ...
            results.record(
                phase,
                f"Content integrity: {pid} has keywords {keywords}",
            ...
            results.record(
                phase,
                f"Content integrity: {pid} body exists",
                False,
                f"{pid} not found in principle bodies",
            )

    # --- Cross-reference integrity (via index) ---
    idx_path = Path.home() / ".claude/.locks/learning-index.json"
    if idx_path.exists():
        _idx = json.loads(idx_path.read_text())
        idx_quotes = _idx.get("quotes", [])
        results.record(
            phase,
            f"Immutable user quotes ({len(idx_quotes)})",
            len(idx_quotes) >= 10,
            f"got {len(idx_quotes)}",
        )

        idx_total = _idx.get("total_learnings", 0)
        results.record(
            phase,
            f"Numbered mistakes ({idx_total})",
            idx_total >= 30,
            f"got {idx_total}",
        )

        idx_graduated = _idx.get("graduated_count", 0)
        results.record(
            phase,
            f"Graduated learnings marked ({idx_graduated})",
            idx_graduated >= 2,
            f"got {idx_graduated}",
        )
        ...
        xref_mistake_nums = set(
            int(m) for m in re.findall(r"^\| (\d+)", bp, re.MULTILINE)
        )
        # Get mistake numbers from index entries (id field)
        learn_mistake_nums = set(
            e.get("id") for e in _idx.get("entries", []) if isinstance(e.get("id"), int)
        )
        ...
        results.record(
            phase,
            f"Xref→learnings coverage ({len(xref_mistake_nums) - len(orphan_xrefs)}/{len(xref_mistake_nums)}, {len(orphan_xrefs)} orphans)",
            len(orphan_xrefs) <= 1,
            f"orphan xrefs: {sorted(orphan_xrefs)[:10]}"
            if len(orphan_xrefs) > 1
            else "",
        )
        ...
        results.record(
            phase,
            f"Antigen→learnings coverage ({len(antigen_nums) - len(orphan_antigens)}/{len(antigen_nums)}, {len(orphan_antigens)} orphans)",
            len(orphan_antigens) <= 3,
            f"orphan antigens: {sorted(orphan_antigens)}"
            if len(orphan_antigens) > 3
            else "",
        )
...
def phase9_scale_and_integration():
    ...
    results.record(
        phase,
        f"70-rule engine built ({total_rules} rules)",
        total_rules >= 70,
        f"got {total_rules}",
    )
    ...
    results.record(
        phase,
        f"Budget enforced at 70 rules (got {len(act70)})",
    ...
    for domain in ["code", "audio", "writing", "git", "advisory", "budget", "scraping"]:
        ...
        results.record(
            phase,
            f"Domain '{domain}' produces activations at scale",
            len(act) > 0,
            f"got 0 activations for {domain}",
        )
    ...
    results.record(
        phase,
        f"Co-activation tracker has {pair_count} pairs (bounded)",
        pair_count < 3000,
    ...
    results.record(
        phase,
        f"Merge detection at scale: {elapsed * 1000:.1f}ms",
        elapsed < 1.0,
    ...
    results.record(
        phase,
        f"Budget analysis: {analysis['avg_eligible']:.1f} eligible, {analysis['avg_loaded']:.1f} loaded",
    ...
    results.record(
        phase,
        f"Activations still work after {dormant_count} rules go dormant",
    ...
    for _ in range(30):
        eng70.advance_day()
    results.record(
        phase, "30-day advance at 70 rules: no crash", True
    ...
    if lh_path.exists():
        ...
        results.record(
            phase,
            "learning_hook.py compiles for integration",
            result.returncode == 0,
            result.stderr[:200],
        )
        ...
        results.record(
            phase,
            "learning_hook.py has injection logic",
            has_injection,
            "no injection pattern found",
        )

        # Hook should produce output (mock test)
        # Run the hook with a simple test payload
        test_payload = json.dumps(
            {"hook_type": "UserPromptSubmit", "prompt": "edit code file"}
        )
        result2 = subprocess.run(
            ["python3", str(lh_path)],
            input=test_payload,
        ...
        results.record(
            phase,
            f"learning_hook.py runs without crash (exit={result2.returncode})",
    ...
    if idx_path.exists():
        ...
        results.record(
            phase,
            f"Seed antigens overlap with learning index ({len(overlap)}/{len(antigen_ids)})",
            len(overlap) >= 3,
