...
def parse_markdown_header(content: str) -> tuple:
    """Parse articles in markdown-header format: # Title, **Author:** ..., **Date:** ..., ---"""
    lines = content.split('\n')
    metadata = {}
    body_start = 0
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.startswith('# ') and 'title' not in metadata:
            ...
            metadata['date_published'] = stripped.replace('**Date:**', '').strip()
        elif stripped.startswith('**URL:**'):
            metadata['source_url'] = stripped.replace('**URL:**', '').strip()
        elif stripped == '---' and i > 0:
            body_start = i + 1
            break
        elif i > 15:  # Stop scanning after 15 lines
            body_start = 0
            break
...
def sync_from_yaml(source_dir: Path, db: KnowledgeDB, source_hint: str = '', skill_hint: str = ''):
    """Sync articles from YAML frontmatter OR markdown-header files into database"""
    synced = 0
    errors = 0
    ...
    for md_file in source_dir.rglob("*.md"):
        ...
        try:
            ...
            if content.startswith('---'):
                ...
                metadata, article_content = parse_markdown_header(content)
                if not metadata.get('title'):
                    continue
            else:
                continue

            # Apply hints if not already in metadata
            if source_hint and not metadata.get('source'):
                metadata['source'] = source_hint
            if skill_hint and not metadata.get('skill'):
                metadata['skill'] = skill_hint
