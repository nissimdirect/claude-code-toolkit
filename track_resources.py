...
def scan_all_sessions():
    ...
    return sorted(sessions, key=lambda x: x["start_time"] or datetime.min, reverse=True)
    ...
    for s in sessions:
        ...
        if s.get("start_time") and s["start_time"] >= window_start:
            ...
            for model_id, model_usage in s["models_used"].items():
                ...
                window_model_msgs[cls] = (
                    window_model_msgs.get(cls, 0) + model_usage["msgs"]
                )
        else:
...
def calculate_monthly_api_equivalent(sessions):
    ...
    month_sessions = [
        s for s in sessions if s["start_time"] and s["start_time"] >= month_start
    ]
    ...
    for s in sessions:
        for model_id, model_usage in s["models_used"].items():
            ...
            if cls not in per_model:
                per_model[cls] = {"wh": 0, "carbon_g": 0, "tokens": 0, "queries": 0}
...
def write_budget_state_json(five_hour, weekly, env, since_gap=None, lifetime=None):
    ...
    try:
        ...
        fd, tmp_path = tempfile.mkstemp(dir=BUDGET_STATE_PATH.parent, suffix=".json")
...
"""
    ...
    for model_id, totals in sorted(model_totals.items()):
        pricing = API_PRICING.get(model_id, API_PRICING["claude-sonnet-4-5-20250929"])
        cost = (
            ...
            * pricing["input"]
            * pricing["cache_create_multiplier"]
            + totals["cache_read"] * pricing["input"] * pricing["cache_read_multiplier"]
        )
        cls = get_model_class(model_id)
        cache_total = totals["cache_create"] + totals["cache_read"]
        report += (
            f"| {cls.title()} | {totals['input']:,} | {totals['output']:,} "
...
def main():
    ...
    output_path = (
        Path.home() / "Development" / "tools" / "output" / "RESOURCE-TRACKER.md"
    )
