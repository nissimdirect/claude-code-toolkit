...
2. **Doing?** promising an action → tool call must follow
3. **Code?** batch text processing → build a script, not burn tokens
4. **Verify?** claiming something works → state the evidence method
5. **Tests?** wrote or modified code → write tests at the RIGHT LAYER: logic/validation → Vitest unit test with mock IPC; component interaction → Vitest component test with mock IPC; process lifecycle/OS integration → Playwright E2E (justify why). See P97.
6. **Reproduce?** fixing a bug → RUN the failing code first, capture the actual error/stack trace. Reasoning about code is NOT enough. You need the real output.
7. **Test Plan?** planning a feature → write a Test Plan section (what to test, edge cases, how to verify) and show it to the user BEFORE coding. For /eng plans: persist to plan file. For ad-hoc work: show inline.
