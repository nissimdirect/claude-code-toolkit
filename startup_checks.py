...
def build_context_package():
    ...
    try:
        result = subprocess.run(
            [
                sys.executable,
                str(HOME / "Development" / "tools" / "build_context_pkg.py"),
                "--validate",
            ],
            capture_output=True,
            text=True,
...
def check_gemini_routing():
    ...
    # 1. Parse template routing from audit log (eval log is stale since v4.4)
    audit_log = LOCKS / "delegation-hook-audit.log"
    if audit_log.exists():
        try:
            lines = [
                l.strip()
                for l in audit_log.read_text().strip().split("\n")
                if l.strip()
            ]

            # Extract template routing entries (action=prefetch_template:*)
            by_cat = {}
            for line in lines:
                for token in line.split():
                    if token.startswith("action=prefetch_template:"):
                        cat = token.split(":", 1)[1]
                        if cat not in by_cat:
                            by_cat[cat] = {"calls": 0, "ok": 0}
                        by_cat[cat]["calls"] += 1
                        if "prefetch=OK" in line:
                            by_cat[cat]["ok"] += 1

            total = sum(v["calls"] for v in by_cat.values())
            ok = sum(v["ok"] for v in by_cat.values())
            est_saved = ok * 2850  # ~2,850 tokens per successful template routing
            ...
            results["eval_log"] = {
                ...
                "top_categories": sorted(
                    by_cat.items(), key=lambda x: x[1]["calls"], reverse=True
                )[:5],
            }
        except Exception as e:
    ...
    try:
        ...
        if r.stdout.strip():
            ...
            disabled = [
                cat
                for cat, info in gate_data.items()
                if info.get("status") == "disabled"
            ]
            warned = [
                cat for cat, info in gate_data.items() if info.get("status") == "warn"
            ]
...
def check_openclaw_exchange():
    ...
    try:
        ...
        return {
            "status": "ok",
            "data": {"new_files": new_files, "count": len(new_files)},
        }
    except Exception as e:
        return {"status": "error", "error": str(e)}
...
def run_workflow_testing_pipeline():
    ...
    results["py_compile"] = {
        "total": len(list(tools_dir.glob("*.py"))),
...
def run_all_checks(run_workflows=False):
    ...
    dirty_repos = [
        k
        for k, v in results.get("repos", {}).get("data", {}).items()
    ...
    gr_quality = (
        results.get("gemini_routing", {}).get("data", {}).get("quality_gate", {})
    )
...
def format_human_readable(data):
    ...
    if li.get("status") == "ok":
        d = li["data"]
        lines.append(
            f"Learning: {d['total']} learnings, {d['graduation_candidates']} graduation candidates"
        )
    else:
        lines.append(
            f"Learning: {li.get('status', 'unknown')} — {li.get('reason', '')}"
        )
    ...
    if con:
        lines.append(
            f"Consistency: {con.get('total', 0)} issues ({con.get('critical', 0)} critical)"
        )
    ...
    if viol.get("total"):
        lines.append(
            f"Violations: {viol['total']} across {viol.get('sessions', '?')} sessions ({viol.get('per_session', '?')}/session), top: {viol.get('top_type', '?')} ({viol.get('top_count', '?')}x)"
        )
    else:
    ...
    if deleg.get("status") == "ok":
        d = deleg["data"]
        lines.append(
            f"Delegation stats: {d['total_prompts']} prompts, {d['total_delegated']} delegated, {d['consecutive_ignored']} ignored"
        )
    ...
    if eval_data.get("total_calls", 0) > 0:
        lines.append(
            f"Gemini routing: {eval_data['total_calls']} calls, {eval_data.get('success_rate', 0)}% success, ~{eval_data.get('est_tokens_saved', 0):,} tokens saved"
        )
    ...
    if daily.get("count", 0) > 0:
        lines.append(
            f"  Gemini API today: {daily['count']}/{daily.get('cap', 200)} ({daily.get('pct_used', 0)}%)"
        )
    lines.append(f"  Templates on disk: {gr.get('templates_on_disk', '?')}")
    qg = gr.get("quality_gate", {})
    if qg.get("disabled"):
        lines.append(
            f"  QUALITY ALERT: {len(qg['disabled'])} template(s) AUTO-DISABLED: {', '.join(qg['disabled'])}"
        )
    if qg.get("warned"):
        lines.append(
            f"  QUALITY WARN: {len(qg['warned'])} template(s) degraded: {', '.join(qg['warned'])}"
        )
    ...
    if sched:
        lines.append(
            f"Workflows due: {sched.get('due_count', 0)} (by tier: {sched.get('by_tier', {})})"
        )
    ...
    if "wf_testing_pipeline" in checks:
        ...
        lines.append(
            f"WF testing-pipeline: {pc.get('total', 0)} files compiled, {len(errors)} errors"
        )
        if errors:
            for e in errors:
    ...
    if "wf_backup_audit" in checks:
        ...
        lines.append(
            f"WF backup-audit: {len(stale)} stale locks, {len(invalid)} invalid state files"
        )
